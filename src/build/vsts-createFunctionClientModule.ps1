...
$parentModule = 'PSLog'
if (-not $ModuleName) { $ModuleName = 'PSLog.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\PSLog'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\PSLog.psd1"
$prereqHash = @{
