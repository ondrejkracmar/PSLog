...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'PSLog.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'PSLog.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'PSLog.Client.ProtectedToken'
